...
SELECT DATE(started_at), YEAR(started_at), QUARTER(started_at), MONTH(started_at), WEEK(started_at), DAY(started_at), HOUR(started_at) FROM  citibike_db.data
...
JOIN citibike_dw.station_dim stn ON A.start_station_id = stn.station_id
